...
class CompCos(nn.Module):
    ...
    def __init__(self, dset, args):
        ...
        self.image_attr_embedder = MLP(dset.feat_dim, args.emb_dim, num_layers= 2, dropout = self.args.dropout,
                norm = self.args.norm, layers = layers, relu = False)     # args.emb_dim(non-codebook) 512(codebook)
        self.image_obj_embedder = MLP(dset.feat_dim, args.emb_dim, num_layers= 2, dropout = self.args.dropout,
                norm = self.args.norm, layers = layers, relu = False)     # args.emb_dim(non-codebook) 512(codebook)
        ...
        self.attr_proj = nn.Linear(input_dim, args.emb_dim)
        self.obj_proj = nn.Linear(input_dim, args.emb_dim)
    ...
    def unique_embed(self, attrs, objs):
        attrs, objs = self.attr_embedder(attrs), self.obj_embedder(objs)
        attrs = F.normalize(self.attr_proj(attrs), dim=1)
        objs = F.normalize(self.obj_proj(objs), dim=1)
        return attrs, objs
    ...
    def val_forward(self, x):
        ...
        if self.args.split_unmatch or self.args.split_loss:
            attr_embs, obj_embs = self.unique_embed(self.uniq_attrs, self.uniq_objs)
        ...
        return score, scores, img_feats_normed
    ...
    def train_forward_open(self, x):
        ...
        loss = loss_cos + split_loss + split_unmatch_loss
