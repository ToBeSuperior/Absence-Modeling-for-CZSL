...
class Evaluator:
    ...
    def generate_predictions(self, scores, obj_truth, bias = 0.0, topk = 5): # (Batch, #pairs)
        ...
        mask = self.oracle_obj_mask[obj_truth.to('cpu')]
    ...
    def score_fast_model(self, scores, obj_truth, bias = 0.0, topk = 5):
        ...
        attr_pred, obj_pred = self.pairs[pair_pred.to('cpu')][:, 0].view(-1, topk), \
            self.pairs[pair_pred.to('cpu')][:, 1].view(-1, topk)
    ...
    def get_accuracies_fast(self, scores, attr_truth, obj_truth, pair_truth, bias=0., seen_mask=None,closed=False):
        ...
        return attr_match.sum(), obj_match.sum(), seen_match, unseen_match, seen_mask, biased_scores
